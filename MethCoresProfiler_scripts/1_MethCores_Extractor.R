...
for (gene1 in 1:length(genes)) {
  ...
  tab=data.frame(Tot_Reads=-1,SampleID=NA)
    ...
    rm(list = ls()[!ls() %in% c("gene_destination_folder","splitForGene","destination_folder","Modify_mono","Meth_unMeth_corr","dectobin","chi_square","flattenCorrMatrix","radian.rescale","quantile","nProcessor","minimal_common_structure","cores_status_assignment",
    ...
    rm(list = ls()[!ls() %in% c("gene_destination_folder","splitForGene","destination_folder","Modify_mono","saveAs","Meth_unMeth_corr","Plot_All","nProcessor","genes","maps","y","pvalue","Reads","Remove","min","maxCG_pos","class",
                                "MethCoresProfiler","input_outputDir","scriptsDir","input","Data","nProcessor1","B4","y1","pvalue1","Reads1","Remove1",
                                "min1","maxCG_pos1","Plot_All1","Meth_unMeth_corr1","saveAs1",
                                "Modify_mono1","splitForGene1","overtime1", "type_Tissue1","population_weight1","Groups1")])
}
