...
#setwd("/Volumes/Avvedimento/DATI_LAB_VEA/Test_pipeline/no2/")
#setwd("/Volumes/G/cloni/")
#setwd("/media/labveacelsius/HD4T/Chiariotti/Mariella/Tissue_PCA/ALL/DDO1/")
setwd("../testData/")

## Install pacman package to manage Rtools
# 1. install.packages("pacman")
# or 
# 2. library(devtools)
# 2. install_github("trinker/pacman")
...
############### Set parameters
#Numero di bootstrapping da fare
y=100
#Set pvalue
pvalue=0.0000000001
...
for (gene1 in 1:length(genes)) {
  ...
  source("../MethCoresProfiler/FUN/tetrachoric_corr_funz.R")
  source("../MethCoresProfiler/FUN/co_Meth_occurence_funz.R")
  source("../MethCoresProfiler/FUN/Mtab_clones_funz.R")
  source("../MethCoresProfiler/FUN/tab_meth_funz.R")
  source("../MethCoresProfiler/FUN/cores_extraction_funz.R")
  source("../MethCoresProfiler/FUN/make_graph_funz.R")
  source("../MethCoresProfiler/FUN/summ_stat_funz.R")
  source("../MethCoresProfiler/FUN/radian.rescale_funz.R")
  source("../MethCoresProfiler/FUN/flattenCorrMatrix_funz.R")
  source("../MethCoresProfiler/FUN/quantile_funz.R")
  source("../MethCoresProfiler/FUN/chi_square_funz.R")
  source("../MethCoresProfiler/FUN/correlation_plot_funz.R")
  source("../MethCoresProfiler/FUN/dectobin_funz.R")
  ...
  ############### Meth_Cores_Extractor ##################################
    ...
    #Crea una lista dei files.out
    outs=list.files(path = "./out/", pattern=".out")
    
    ## fot multiple regions use "grep gene" belòw
    #outs1=outs[grep(gene, outs, fixed=T)]
    outs1=paste("./out/",outs,sep = "")
    #file="p15R2_Tramontano1m.out"
    #Carica un singolo file per fare la prova dello script
    tab=data.frame(Tot_Reads=-1,SampleID=NA)
    ####################### rarefaction 
    for (out in 1:length(outs1)) { 
      #Carica il file.out dalla lista files
      experiment <- read.delim(outs1[out], quote="\"", stringsAsFactors = F)
      
      #Calcola il numero di righe (profili totali) del campione caricato
      ...
      #Dividi il nome del campione che hai caricato
      
      a=strsplit(outs1[out], "\\.")[[1]]
      a1=strsplit(a, "\\/")[[2]]
      #Salva il nome del campione come "sam"
    ...
    #Salva il numero di files all'interno della lista
    tab=tab[!is.na(tab$SampleID),]
    
    #elimina le righr contenenti caratteri alfanumerici
    b=subset(tab, grepl('^\\d+$', tab$Tot_Reads))
    
    #Converti i caratteri in numeri
    ...
    if (Reads=="average") {
      #calcola la media del dataframe "b"
      b4=round(as.numeric(colMeans(b, na.rm = FALSE, dims = 1)),digits = 0)
    } else { 
      b4=round(as.numeric(b[1,1]),digits = 0)
    }
    ...
    tab=tab[ order(match(tab$SampleID, map$X.SampleID)), ]
    
    write.table(x = tab, file = paste("Table_Reads_",gene,".txt",sep=""), quote=F,sep="\t",row.names=F)
    
    #Cancella tutto e lascia solo i file.out
    rm(list = ls()[!ls() %in% c("dectobin","chi_square","flattenCorrMatrix","radian.rescale","quantile","nProcessor","minimal_common_structure","cores_status_assignment",
                                ...
                                "correlation_plot","CG_pos","gene","map","Expected_Freq","Plot_All","b4","Random","outs1")])
    #carica un file.out per fare una prova
    #out="DDO1_M1_0.out"
    ########################## tab_clone_function 
    #Crea df for
    ...
    for (out in 1:length(outs1)) {
      if (outs1[out]=="Random") {
       ...
       # names(Random) <- paste0(1:length(Random))
        ...
        sam1=strsplit(x = outs1[out], split = "/") [[1]]
        sam1=strsplit(x = sam1, split = ".out") [[3]]
        sample=map[map$SampleID %in% sam1,]
        
        if (sample$Description==sample$Group) {
      ...
      png(paste(sam, "_Tetrachoric_Correlation_", gene,".png", sep=""), width=5*300,height=5*300, res = 300,pointsize=8)
      ...
      rm(list = ls()[!ls() %in% c("dectobin","chi_square","flattenCorrMatrix","radian.rescale","quantile","nProcessor","minimal_common_structure","cores_status_assignment",
                                  "cores_heatmap","epialleles_heatmap","strings_to_BinaryProfiles","complexity","compare_Samples_epialleles","combs_summ_clust1",
                                  "combs_summary1","combinations","summ_stat","make_graph","cores_extraction","tab_meth","Mtab_clones","co_Meth_occurence","tetrachoric_corr",
                                  "nProcessor","genes","maps","Complexity","y","pvalue","Reads","Remove","min","maxCG_pos","class","CG_pos","gene","map","Expected_Freq","Plot_All",
                                  "correlation_plot","b4","Random","ner2","Mcor","MShannon_entropy_tab","Mtaxa_tab","MRenyi_entropy_tab","Mepiallelic_tab","outs1")])
    }
    
    #################################### BarPlot metilazione media campione con SE 
    ...
    write.table(x = Mtaxa_tab, file = paste("Table_Stat_Taxa_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = Mcor, file = paste("Table_Stat_Pearson_Correlation_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = MShannon_entropy_tab, file = paste("Table_Stat_Shannon_Entropy_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = MRenyi_entropy_tab, file = paste("Table_Stat_MRenyi_Entropy_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = ner2, file = paste("Table_Stat_Methylation_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    for (t in 1:length(Tissue)) {
      ...
      write.table(x=tab2, file=paste("Methylation_Plotter_",Tissue[t],"_",gene, ".txt", sep=""), row.names=T, col.names = F,quote = F, sep="\t")
      ...
      if (saveAs=="pdf") {
        pdf(paste("Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x=aggregateGroups, file=paste("Table_Stat_Average_Methylation_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      if (saveAs=="pdf") {
        pdf(paste("Average_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Average_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      if (saveAs=="pdf") {
        pdf(paste("CpGs_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("CpGs_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      #Save Average Methylation
      write.table(x=aggregateCpGsGroups, file=paste("Table_Stat_Average_Methylation_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste("Average_CpGs_Meth+se_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Average_CpGs_Meth+se_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      if (saveAs=="pdf") {
        pdf(paste("Average_CpGs_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Average_CpGs_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x = taxa1, file = paste("Table_Taxonomy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      ...
      if (saveAs=="pdf") {
        pdf(paste("Taxonomy_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Taxonomy_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x=cor3, file=paste("Table_Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".txt", sep=""), row.names=T, col.names = F,quote = F, sep="\t")
      
      if (saveAs=="pdf") {
        pdf(paste("Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".png", sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x = MShannon_entropy_tab, file = paste("Table_Stat_Shannon_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      ...
      write.table(x = Shannon_Entropy1, file = paste("Table_Shannon_Entropy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste("Shannon_Entropy_",Tissue[t],"_",class,"_",gene,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Shannon_Entropy_",Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
      }
      ...
      #Save Average Methylation
      write.table(x=aggregateGroups, file=paste("Table_Stat_Average_Shannon_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      #Average Shannon plot
      if (saveAs=="pdf") {
        pdf(paste("Average_Shannon_Entropy_", Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Average_Shannon_Entropy_", Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      jj=ggplot(aggregateGroups, aes(x=Timing, y=mean,fill=Timing)) + 
        geom_bar(position=position_dodge(), stat="identity",
                 colour="black", # Use black outlines,
      ...
      write.table(x = Renyi_Entropy1, file = paste("Table_Renyi_Entropy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste("Renyi_Entropy_",Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Renyi_Entropy_",Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
      }
      ...
      aggregateGroups=merge(aggregateGroups,aggregateGroups_sd,by=c("Timing","Tissue"),all=TRUE)
      #Save Average Methylation
      write.table(x=aggregateGroups, file=paste("Table_Stat_Average_Renyi_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      #Average Shanno plot
      if (saveAs=="pdf") {
        pdf(paste("Average_Renyi_Entropy_", Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste("Average_Renyi_Entropy_", Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      jj=ggplot(aggregateGroups, aes(x=Timing, y=mean,fill=Timing)) + 
        geom_bar(position=position_dodge(), stat="identity",
                 colour="black", # Use black outlines,
    ...
    rm(list = ls()[!ls() %in% c("saveAs","Meth_unMeth_corr","Plot_All","nProcessor","genes","maps","y","pvalue","Reads","Remove","min","maxCG_pos","class")])
}
