...
source(paste(scriptsDir,"/FUN/tetrachoric_corr_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/co_Meth_occurence_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/Mtab_clones_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/tab_meth_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/cores_extraction_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/make_graph_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/summ_stat_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/radian.rescale_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/flattenCorrMatrix_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/quantile_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/chi_square_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/co_Meth_unMeth_occurence_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/co_unMeth_Meth_occurence_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/correlation_plot_funz.R",sep=""))
source(paste(scriptsDir,"/FUN/dectobin_funz.R",sep=""))
...
b4=B4 #"default" or "fix a number, example:10000"
y=y1
#Set pvalue
pvalue=pvalue1
#Set average reads or smallest n reads
Reads=Reads1 ## "smallest"
Remove=Remove1  #yes
min=min1
...
Meth_unMeth_corr=Meth_unMeth_corr1#"YES" #or "NO
saveAs="saveAs1"#pdf or png
Modify_mono=Modify_mono1 #"TRUE" or "FALSE" #if TRUE modify monomethyl
splitForGene=splitForGene1 #"splitForGene",
#Set n processors
...
dir.create(paste(input_outputDir,"2CpGs/",sep=""))
destination_folder=paste(input_outputDir,"2CpGs/",sep="") #"destination_folder",
class="Methylated"
...
for (gene1 in 1:length(genes)) {
  ...
  outs=list.files(path = paste(input_outputDir,"/",input,sep=""), pattern=".out")
  ...
  #outs1=paste(input_outputDir,"/",input,"/",outs1,sep="")
  ...
  info = file.info(paste(input_outputDir,"/",input,"/",outs1[out],sep=""))
  if (info$size == 0) {
    empty = append(empty,outs1[out])
    #outs1=outs1[!grepl(outs1[out], outs1, fixed=T)]
    #Modify map
    write.table(x = map, file = paste(input_outputDir,"/",metaCGpos,"/original_",gene,"_meta.map",sep=""), quote=F,sep="\t",row.names=F)
    map=map[!grepl(sub(".out","",outs1[out]), map$SampleID),]
    write.table(x = map, file = paste(input_outputDir,"/",metaCGpos,"/",gene,"_meta.map",sep=""), quote=F,sep="\t",row.names=F)
  } else {
    empty=empty
    map=map
    outs1=outs1
    ...
    outs1=setdiff(as.character(outs1),as.character(empty))
    ####################### rarefaction 
    for (out in 1:length(outs1)) { 
    ...
    write.table(x = tab, file = paste(gene_destination_folder,"/","Table_Reads_",gene,".txt",sep=""), quote=F,sep="\t",row.names=F)
    
    #Delete everything and leave only the.out files
    rm(list = ls()[!ls() %in% c("RAM","metaCGpos","HighComplexity1","vertex_cutOFF1","gene_destination_folder","splitForGene","destination_folder","Modify_mono","Meth_unMeth_corr","dectobin","chi_square","flattenCorrMatrix","radian.rescale","quantile","nProcessor","minimal_common_structure","cores_status_assignment",
    ...
    colnames(MShannon_entropy_tab)=map$SampleID
    
    MRenyi_entropy_tab=data.frame(matrix(ncol = (dim(map)[1]), nrow = 0))
    colnames(MRenyi_entropy_tab)=map$SampleID
    ...
    for (out in 1:length(outs1)) {
      if (outs1[out]=="Random") {
       ...
       # names(Random) <- paste0(1:length(Random))
        ...
        sam1=strsplit(x = outs1[out], split = ".out") [[1]][1]
        
        if (sam1 %in% map$SampleID) {
          sample=map[map$SampleID %in% sam1,]
      } else {
        sam1=sub(paste(gene,"_",sep=""),"",sam1)
        sample=map[map$SampleID %in% sam1,]
      }
        #sample=map[map$SampleID %in% sam1,]
        if (sample$Tissue!=sample$Group) {
        sam=paste(sample$Tissue,sample$Group,sample$Description,sample$Rep,sep="_")
        }else{
          sam=paste(sample$Tissue,sample$Description,sample$Rep,sep="_")
        }
      ...
      tab1=foreach(i=1:y,.packages=c("stringr","psych","dplyr","purrr")) %dopar% {
        tab1[[i]] = tab_meth(sam = sam, CG_pos = CG_pos,b4=b4,input_outputDir,input)
      }
      #Epiaplotypes (Methylated profiles) tab
      tcs=foreach(i=1:y,.packages=c("stringr","psych","dplyr","purrr")) %dopar% {
        tcs[[i]] = Mtab_clones(sam = sam,CG_pos = CG_pos, b4=b4,input_outputDir,input)
      }
      #tetrachoric tab
      tetrachoric=foreach(i=1:y,.packages=c("stringr","psych","dplyr","purrr")) %dopar% {
        tetrachoric[[i]]= tetrachoric_corr(sam = sam, CG_pos = CG_pos,b4=b4,input_outputDir,input)
      }
      #co_Meth tab
      mclust=foreach(i=1:y,.packages=c("RColorBrewer","corrplot","dplyr","purrr")) %dopar% {#.options.snow=opts
        mclust[[i]]= co_Meth_occurence(sam = sam,CG_pos = CG_pos,b4=b4,input_outputDir,input)
      }
      
      if (Meth_unMeth_corr=="YES") {
      #co_Meth_unMeth tab
      Meth_unMethclust=foreach(i=1:y,.packages=c("RColorBrewer","corrplot","dplyr","purrr")) %dopar% {#.options.snow=opts
        Meth_unMethclust[[i]]= co_Meth_unMeth_occurence(sam = sam,CG_pos = CG_pos,b4=b4,input_outputDir,input)
      }
      #co_unMeth_Meth tab
      unMeth_Methclust=foreach(i=1:y,.packages=c("RColorBrewer","corrplot","dplyr","purrr")) %dopar% {#.options.snow=opts
        unMeth_Methclust[[i]]= co_unMeth_Meth_occurence(sam = sam,CG_pos = CG_pos,b4=b4,input_outputDir,input)
      }
      
      ################# Meth_unMeth_correlation
      MethCorr=correlation_plot(tclust=mclust,gene_destination_folder)
      Meth_unMethCorr=correlation_plot(tclust=Meth_unMethclust,gene_destination_folder)
      unMeth_MethCorr=correlation_plot(tclust=unMeth_Methclust,gene_destination_folder)
      } else {
        ################# Meth_unMeth_correlation
        MethCorr=correlation_plot(tclust=mclust,gene_destination_folder)
      }
      ...
      if (saveAs=="pdf") {
      pdf(paste(gene_destination_folder,"/",sam, "_Tetrachoric_Correlation_", gene,".pdf", sep=""),paper = "a4")
      } else {
      png(paste(gene_destination_folder,"/",sam, "_Tetrachoric_Correlation_", gene,".png", sep=""), width=5*300,height=5*300, res = 300,pointsize=8)
      }
      ...
      meth_list=cores_extraction(ttcs=tcs,tclust=mclust,gene_destination_folder)
      
      mer=meth_list$mer
      mer2=meth_list$mer2
      g=make_graph(tmer=mer,tmer2=mer2,sam=sam,summ_stat=summ_stat,gene_destination_folder)
    ...
    ## remove row if colun contain specific string
    #ner3=ner2[!grepl("Random", df$Name),]
    #ner2=ner2[-c(1),]
    #Mtaxa_tab=Mtaxa_tab[-c(296:309),]
    #Mcor=Mcor[,-c(23)]
    #colnames(Mcor)[22]="Random"
    #MShannon_entropy_tab=MShannon_entropy_tab[-c(22),]
    #MRenyi_entropy_tab=MRenyi_entropy_tab[-c(22),]
    #################################### BarPlot sample mean methylation with SE 
    #Per fare il grafico a parte
    #ner2=read.table(paste(gene_destination_folder,"/Table_Stat_Methylation_",gene,".txt",sep=""),header = T, check.names = F)
    ...
    write.table(x = Mtaxa_tab, file = paste(gene_destination_folder,"/","Table_Stat_Taxa_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = Mcor, file = paste(gene_destination_folder,"/","Table_Stat_Pearson_Correlation_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = MShannon_entropy_tab, file = paste(gene_destination_folder,"/","Table_Stat_Shannon_Entropy_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = MRenyi_entropy_tab, file = paste(gene_destination_folder,"/","Table_Stat_MRenyi_Entropy_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    write.table(x = ner2, file = paste(gene_destination_folder,"/","Table_Stat_Methylation_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
    ...
    for (t in 1:length(Tissue)) {
      ...
      if (unique(SamplesID$Tissue==SamplesID$Group)==TRUE) {
      SamplesID$order=paste(SamplesID$Tissue, SamplesID$Description, SamplesID$Rep,sep="_")
      } else {
        SamplesID$order=paste(SamplesID$Tissue,SamplesID$Group,SamplesID$Description, SamplesID$Rep,sep="_")
      }
      ...
      write.table(x=tab2, file=paste(gene_destination_folder,"/","Methylation_Plotter_",Tissue[t],"_",gene, ".txt", sep=""), row.names=T, col.names = F,quote = F, sep="\t")
      ...
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x=aggregateGroups, file=paste(gene_destination_folder,"/","Table_Stat_Average_Methylation_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Average_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Average_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      ner3$Samples=sub("_[^_]+$","",ner3$Samples)
      #ner3$Samples=sub("_r.*","",ner3$Samples)
      ...
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","CpGs_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","CpGs_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      #Save Average Methylation
      write.table(x=aggregateCpGsGroups, file=paste(gene_destination_folder,"/","Table_Stat_Average_Methylation_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Average_CpGs_Meth+se_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Average_CpGs_Meth+se_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Average_CpGs_Meth_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Average_CpGs_Meth_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x = taxa1, file = paste(gene_destination_folder,"/","Table_Taxonomy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      ...
      #taxa1=taxa1[match(gsub("^.*?_","",map$id), taxa1$Samples),]
      ...
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Taxonomy_",Tissue[t],"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Taxonomy_",Tissue[t],"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      rm(taxa1,Random)
      ################## Samples Correlation
      cor1=Mcor[,c("id_pos",SamplesID$order)]
      ...
      write.table(x=cor3, file=paste(gene_destination_folder,"/","Table_Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".txt", sep=""), row.names=T, col.names = F,quote = F, sep="\t")
      
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Pearson_Correlation_matrix_",Tissue[t],"_",class,"_",gene,".png", sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      ...
      write.table(x = MShannon_entropy_tab, file = paste(gene_destination_folder,"/","Table_Stat_Shannon_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      ...
      write.table(x = Shannon_Entropy1, file = paste(gene_destination_folder,"/","Table_Shannon_Entropy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Shannon_Entropy_",Tissue[t],"_",class,"_",gene,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Shannon_Entropy_",Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
      }
      ...
      #Save Average Methylation
      write.table(x=aggregateGroups, file=paste(gene_destination_folder,"/","Table_Stat_Average_Shannon_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      #Average Shannon plot
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Average_Shannon_Entropy_", Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Average_Shannon_Entropy_", Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      jj=ggplot(aggregateGroups, aes(x=Timing, y=mean,fill=Timing)) + 
        geom_bar(position=position_dodge(), stat="identity",
                 colour="black", # Use black outlines,
      ...
      write.table(x = Renyi_Entropy1, file = paste(gene_destination_folder,"/","Table_Renyi_Entropy_",Tissue[t],"_",class,"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Renyi_Entropy_",Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Renyi_Entropy_",Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
      }
      ...
      aggregateGroups=merge(aggregateGroups,aggregateGroups_sd,by=c("Timing","Tissue"),all=TRUE)
      #Save Average Methylation
      write.table(x=aggregateGroups, file=paste(gene_destination_folder,"/","Table_Stat_Average_Renyi_Entropy_",Tissue[t],"_",gene, ".txt",sep=""), quote=F,sep="\t",row.names=F)
      
      aggregateGroups$Timing <- factor(aggregateGroups$Timing, levels = unique(map$Description))
      
      #Average Shanno plot
      if (saveAs=="pdf") {
        pdf(paste(gene_destination_folder,"/","Average_Renyi_Entropy_", Tissue[t],"_",class,"_",gene,".pdf", sep=""),paper = "a4")
      } else {
        png(paste(gene_destination_folder,"/","Average_Renyi_Entropy_", Tissue[t],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8)
      }
      jj=ggplot(aggregateGroups, aes(x=Timing, y=mean,fill=Timing)) + 
        geom_bar(position=position_dodge(), stat="identity",
                 colour="black", # Use black outlines,
    ...
    #rm(list = ls()[!ls() %in% c("vertex_cutOFF1","gene_destination_folder","splitForGene","destination_folder","Modify_mono","saveAs","Meth_unMeth_corr","Plot_All","nProcessor","genes","maps","y","pvalue","Reads","Remove","min","maxCG_pos","class",
    #                            "MethCoresProfiler","input_outputDir","scriptsDir","input","Data","nProcessor1","B4","y1","pvalue1","Reads1","Remove1",
    #                            "min1","maxCG_pos1","Plot_All1","Meth_unMeth_corr1","saveAs1",
    #                            "Modify_mono1","splitForGene1","overtime1", "type_Tissue1","population_weight1","Groups1")])
