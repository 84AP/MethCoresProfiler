...
HighComplexity=HighComplexity1 #FALSE #TRUE or FALSE
...
MethCores=data.frame(average_methylation=-1,MethCores_Index=-1,Clonality_Index=-1,Group=NA,Groups=NA,Tissue=NA,Gene=NA,MethCores=NA,quantile=-1,sd_MethCores_Index=-1,se_MethCores_Index=-1,sd_Clonality_Index=-1,se_Clonality_Index=-1,MoreRepresentativeEpiallele=-1)
MethCores=MethCores[0,]
...
for (gene1 in 1:length(genes)) {
  ...
  #registerdoSNOW(cl)
  #registerDoParallel(nProcessor)
  #clusterExport(nProcessor, genes[gene1])
  #on.exit(stopCluster(nProcessor))
  #stopCluster(nProcessor)
  ...
  if (unique(map$Tissue==map$Group)==TRUE) {
  map$id=paste(map$Group,map$Description,map$Rep,sep="_")
  } else {
  ...
  repeat {
    colorCodes <-palette(rainbow(length(unique(tmap$Group)))) #length(Groups[-(length(Groups))])
    if (length(colorCodes)==length(unique(tmap$Group))) {
      break
      print("colorCodes OK!!")
    }
  }
...
#    }
  ...
  write.table(x=MethCores1, file=paste(gene_destination_folder,"/Table_MethCoresProfiler_",gene,".txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
  
  MethCores=rbind(MethCores,MethCores1)
  #rm(MethCores1)
  
  
  #rm(list = ls()[!ls() %in% c("MethCores","vertex_cutOFF1","HighComplexity","input_folder1","input_folder2","type_Tissue","destination_folder","FOLDERS","colorCodes","overtime","population_weight","MethCores_tab2","saveAs","destination_folder","select","exclude","Plot_All","MethCores_tab","MethCores_tab2","Assign_color","CG_pos","Complex","Complex1","Group","tmap","Random_control","Selection","pvalue",
  ...
  #rm(colorCodes)
  rm(list = ls()[!ls() %in% c("RAM","nProcessor","MethCores","metaCGpos","HighComplexity1","complexity","genes","maps",
  "maxCG_pos","Plot_All","overtime","saveAs","Groups","type_Tissue","input_folder1","input_folder2",
  "destination_folder","Complex","HighComplexity","population_weight","cores_cutOff",
  "vertex_cutOff","class","class1","Selection","exclude","select",
  "vertex_cutOFF1","MethCoresProfiler","input_outputDir","scriptsDir","input","Data","nProcessor1",
  "B4","y1","pvalue1","Reads1","Remove1","min1","maxCG_pos1","Plot_All1","Meth_unMeth_corr1","saveAs1",
  "Modify_mono1","splitForGene1","overtime1", "type_Tissue1","population_weight1","Groups1")])
  write.table(x=MethCores, file=paste(destination_folder,"/Table_MethCoresProfiler.txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
