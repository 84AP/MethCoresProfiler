compare_Samples_epialleles <- function(LIST,Random_summ1,map,input_folder2) {
  ...
  em=foreach(h=1:length(LIST), .combine= merge.by.time) %dopar% {
    ...
    #}
      ...
      #load file by name
      summary=read.table(paste("./",input_folder2,"/",LIST[h],sep=""), header=TRUE, sep="\t",stringsAsFactors=FALSE)
