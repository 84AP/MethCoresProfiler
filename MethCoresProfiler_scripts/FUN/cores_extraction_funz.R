cores_extraction <- function(mer,ttcs,tclust) {
  ...
  write.table(x = mer, file = paste(gene_destination_folder,"/",sam,"_",class,"_",gene,"_clones.txt", sep=""), quote = F, sep = "\t", row.names = F, col.names = T)
  ...
  if (saveAs=="pdf") {
    pdf(paste(gene_destination_folder,"/",sam,"_",class,"_Taxa_",gene,".pdf", sep=""),paper = "a4")
  } else {
  png(paste(gene_destination_folder,"/",sam,"_",class,"_Taxa_",gene,".png", sep=""), width=5*150,height=5*150, res = 100,pointsize=4)
  }
  ...
  if (saveAs=="pdf") {
    pdf(paste(gene_destination_folder,"/",sam,"_co_",class,"_Occurence_", gene,".pdf", sep=""),paper = "a4")
  } else {
  png(paste(gene_destination_folder,"/",sam,"_co_",class,"_Occurence_", gene,".png", sep=""), width=5*300,height=5*300, res = 300,pointsize=8)
  }
