...
cores_extraction <- function(mer,ttcs,tclust) {
  ...
  #Dividi per il numero di matrici di tcs
  ...
  write.table(x = mer, file = paste(sam,"_",class,"_",gene,"_clones.txt", sep=""), quote = F, sep = "\t", row.names = F, col.names = T)
  
  #Correlation matrix building
  cor=mer[,c("id_pos","FreqCloneForSample")]
  colnames(cor)[2]=sam
  
  #Disegna binominal distributio
  ...
  if (saveAs=="pdf") {
    pdf(paste(sam,"_",class,"_Taxa_",gene,".pdf", sep=""),paper = "a4")
  } else {
  png(paste(sam,"_",class,"_Taxa_",gene,".png", sep=""), width=5*150,height=5*150, res = 100,pointsize=4)
  }
  ...
  if (saveAs=="pdf") {
    pdf(paste(sam,"_co_",class,"_Occurence_", gene,".pdf", sep=""),paper = "a4")
  } else {
  png(paste(sam,"_co_",class,"_Occurence_", gene,".png", sep=""), width=5*300,height=5*300, res = 300,pointsize=8)
  }
