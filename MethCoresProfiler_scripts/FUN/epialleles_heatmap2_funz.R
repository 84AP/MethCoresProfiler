epialleles_heatmap2 <- function(Epi_list,Stat,vertex_cutOFF,colorCodes,input_folder2,HighComplexity,Random,gene,gene_destination_folder,Groups) {
  ...
  for (hh in 1:length(Complexity))   {
    ...
    em=compare_Samples_epialleles(LIST=Epi_list1,Random_summ1=Random_summ1,map=map,input_folder2=input_folder2,gene=gene)
    ...
    for (g in 1:length(Groups)) {
      ...
      write.table(x=dim2, file=paste(gene_destination_folder,"/Table_Positive_Epialleles_",Groups[g],"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
      ...
      write.table(x=dim, file=paste(gene_destination_folder,"/Table_Dendrogram_Epialleles_",Groups[g],"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
      
      #tab_Percento Epiallels
      dim1=data1
      dim1=dim1[match(rownames(dim), rownames(dim1)),]
      write.table(x=dim1, file=paste(gene_destination_folder,"/Table_Freq_Epialleles_",Groups[g],"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T, col.names=T, quote = F, sep="\t")
      ...
      #writeDist(parallelDist(dim,method="binary", threads = nProcessor1),file=paste(gene_destination_folder,"/Binary_Dist_",Groups[g],"_",gene,".txt", sep=""))
      #rm(Dist)
      gc()
      #hDist=fastcluster::hclust(readDist(paste(gene_destination_folder,"/Binary_Dist_",Groups[g],"_",gene, sep="")))
      #row_dend=as.dendrogram(hDist)
      #file.remove(paste(gene_destination_folder,"/Binary_Dist_",Groups[g],"_",gene, sep=""))
      #row_dend =as.dendrogram(fastcluster::hclust(parallelDist(dim,method="binary", threads = nProcessor1)))#, threads = nProcessor
      #
      #gc()
      ...
      if (Groups[g]=="All") {
        ...
        png(paste(gene_destination_folder,"/Clustering_CpGs_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
        ...
        #}
        png(paste(gene_destination_folder,"/Clustering_EpiallelesFreq_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
            width = 5*250,        # 5 x 300 pixels
            height = 5*250,
            res = 300,            # 300 pixels per inch
            pointsize = 4)        # smaller font size
        ...
        png(paste(gene_destination_folder,"/Clustering_Selection_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
            width = 5*250,        # 5 x 300 pixels
            height = 5*250,
            res = 300,            # 300 pixels per inch
            pointsize = 4)        # smaller font size
      ...
      png(paste(gene_destination_folder,"/Clusters_Epialleles_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
          width = 5*250,        # 5 x 300 pixels
          height = 5*250,
          res = 300,            # 300 pixels per inch
          pointsize = 4)        # smaller font size
      ...
      if (Complexity[hh]=="dimethyl") {
        MethCores=cores_status_assignment(str1=dim1,str2=dim2,Complexity=Complexity,hh=hh,g=g,Group=Group,tmap1=tmap1,maxCG_pos=maxCG_pos,
                                          Stat=Stat,vertex_cutOFF=vertex_cutOFF,dim1=dim1,Epi_list1=Epi_list1,gene_destination_folder,Groups) 
        
        if (dim(MethCores$MethCores)[1]!=0) {
    ...
    ##Dendrogram only for if Group==ALL
        if (Groups[g]=="All") {
          ...
          # if(type_Tissue=="same") {
          # hcl$labels=gsub(".*_","",hcl$labels)
          # } else {
          #   hcl$labels=hcl$labels
          # }
          ...
          #if (length(d)>1) {
           png(paste(gene_destination_folder,"/Phylogenetic_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
              width = 5*250,        # 5 x 300 pixels
              height = 5*250,
        ...
        ##Write phylogram
         ...
         write.hclust(hcl, file =  paste(gene_destination_folder,"/Phylogenetic_Cluster_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".txt",sep=""), prefix = prefix,h=max(hcl$height)) 
         write.table(x=dm, file=paste(gene_destination_folder,"/Table_Phylogenetic_Cluster_",Groups[g],"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
         
          
          ### Dendrogram with all Freq
         nm1=as.data.frame(t(nm[,3:dim(nm)[2]]))
         colnames(nm1)=as.character(nm[,1])
         #nm1=nm1[-c(1:2),]
         ...
         for (nnnn in 1:nrow(nm1)) {
          # if (str_count(rownames(nm1)[nnnn],"\\_")>2) {
             ...
             rownames(nm1)[nnnn]=paste(SAM1[(length(SAM1)-2)],SAM1[(length(SAM1)-1)],SAM1[(length(SAM1))],sep="_")
           #} else {
        ...
        # }
         ...
         if (length(hc)>1) {
         # And the plot
         png(paste(gene_destination_folder,"/Dendrogram_",Groups[g],"_FreqEpiallels_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
             width = 5*250,        # 5 x 300 pixels
             height = 5*250,
             res = 500,            # 300 pixels per inch
             pointsize = 4,units = c("px"))        # smaller font size
       ...
       dev.off()
        ...
        #if (length(d)>1) {
        png(paste(gene_destination_folder,"/Dendrogram_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
            width = 5*250,        # 5 x 300 pixels
            height = 5*250,
            res = 500,            # 300 pixels per inch
            pointsize = 4,units = c("px"))        # smaller font size
        ...
        write.hclust(h, file =  paste(gene_destination_folder,"/Dendrogram_Cluster_",Groups[g],"_",class,"_",Complexity[hh],"_",gene,".txt",sep=""), prefix = prefix,h=max(hcl$height)) 
        write.table(x=mat, file=paste(gene_destination_folder,"/Table_Dendrogram_Cluster_",Groups[g],"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
        #}
        }
        ...
        write.table(x=  tab_MethCores, file=paste(gene_destination_folder,"/Table_CpG_Contribution_",Groups[g],"_",class,"_",gene,".txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
        ...
        colnames(yearly_counts)[2]="Groups"
        ...
        GGroups=as.character(unique( yearly_counts$Groups))
        
        if ((type_Tissue=="same")==TRUE) { 
          #GGroups=unique(yearly_counts$Groups)
          
          png(paste(gene_destination_folder,"/MethCores_Entanglement_plot_",Groups[g],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 250,pointsize = 8) 
          
          j2=ggplot(yearly_counts,aes(X,CpG_Variation, group=CpGs)) + geom_point(aes(colour=CpGs)) + geom_line(aes(colour=CpGs))+
            facet_wrap(~ Groups, scales="free_x") + #~ Family
            ylim(0,1)+
            xlab(X1) +
        ...
        rm(list = ls()[!ls() %in% c("RAM","gene_destination_folder","gene","HighComplexity","input_folder1","input_folder2","type_Tissue","destination_folder","FOLDERS","colorCodes","destination_folder","overtime","Epi_list1","population_weight","MethCores_tab2","cores_cutOff","MethCoresIndex","MethCores_tab","Assign_color","MethCores1","MethCores",
