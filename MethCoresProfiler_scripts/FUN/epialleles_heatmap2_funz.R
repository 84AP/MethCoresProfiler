...
epialleles_heatmap2 <- function(Epi_list,Stat,vertex_cutOFF,colorCodes) {
  ...
  for (hh in 1:length(Complexity))   {
    ...
    if ((Random_summ1 %in% Epi_list1)==FALSE) next
    ...
    for (g in 1:length(Groups)) {
      ...
      if (Groups[g]=="All") {
        ...
        tmap1=tmap[tmap$Tissue %in% Groups[g],]
        Random1=tmap[tmap$Group %in% "R",]
      ...
      lgd2 = Legend(at = c("positive selection", "negative selection","no selection"),title = "Types of Selection",  labels_gp = gpar(fontsize = 5), legend_gp = gpar(fill = c("red","blue","white")),border = TRUE,
                    title_gp = gpar(fontsize = 5, fontface = "bold"))
      lgd3 = Legend(at = colorCodes1$Samples,title = "Samples",  labels_gp = gpar(fontsize = 5), legend_gp = gpar(fill =c(colorCodes1$colors)),border = TRUE,
                    title_gp = gpar(fontsize = 5, fontface = "bold"))
      pd = packLegend(lgd, lgd1,lgd2,lgd3,direction = "vertical")
      #ha =  HeatmapAnnotation(df = data.frame(samples = sample(unique(sub("_.*","",colnames(dim1))))),col = list(samples = colorCodes))
      ann <- data.frame(Samples=c(sub("_.*","",colnames(dim1))))
      colours=list(Samples=c("Cond1"="red", "Cond2"="green", 
                             "Cond3"="yellow3","Cond4"="blue",
                             "R1"="cornflowerblue","R2"="darkorchid3","R"="black"))
      ha <- HeatmapAnnotation(df=ann, border = TRUE, which="col", show_annotation_name = F,col=colours, annotation_width=unit(c(.5, .5), "cm"), gap=unit(.5, "mm"),show_legend = F)           
      ## Clustering matrix
      ...
      if (Groups[g]=="All") {
        ...
        for (g_2 in 1:length( Groups_List)) {
          ...
          dim_m1=Heatmap(dim_m,cluster_rows = row_dend, name = Groups_List[g_2],show_column_names = T,show_row_names = FALSE,border = TRUE,show_heatmap_legend = FALSE,show_row_dend = FALSE,
                         column_names_gp = gpar(fontsize =  5, fontface = "bold"),cluster_columns = FALSE,col = col_fun,column_title=Groups_List[g_2],column_title_gp =gpar(fontsize =  5, fontface = "bold"))
          #heatmap_legend_param = list(title = paste("Frequency of Epiallels ",Groups_List[g2],sep=""), title_gp = gpar(fontsize = 5, fontface = "bold"),labels_gp = gpar(fontsize = 5)))
        ...
        for (g_3 in 1:length( Groups_List)) {
          ...
          dim_m2=Heatmap(dimm_m,cluster_rows = row_dend, name = Groups_List[g_3],show_row_names = FALSE, show_column_names = T,border = TRUE,show_heatmap_legend = FALSE,show_row_dend = FALSE,
                         column_names_gp = gpar(fontsize =  5, fontface = "bold"), cluster_columns = FALSE,col=col_dim2,column_title=Groups_List[g_3], column_title_gp =gpar(fontsize =  5, fontface = "bold")) #col = col_fun1,
          #heatmap_legend_param = list(title = paste("Types of Selection ",Groups_List[g3],sep=""),title_gp = gpar(fontsize = 5, fontface = "bold"),
          #                           labels_gp = gpar(fontsize = 5)))
        ...
        dendro1 <- Heatmap(dim, cluster_rows = row_dend, name = "CpG Methylation",width = unit(1, "cm"), show_row_names = FALSE, show_column_names = T, border = TRUE,col = c("white", "red"),
                           column_names_gp = gpar(fontsize =  5, fontface = "bold"),row_dend_width = unit(1, "cm"),cluster_columns = FALSE, show_heatmap_legend = F, row_title = "Cluster",)+
          Heatmap(dim1,cluster_rows = row_dend, name = "Frequency of Epiallels",show_column_names = F,show_row_names = FALSE,border = TRUE,show_heatmap_legend = FALSE,show_row_dend = FALSE,
          ...
          Heatmap(dim2,cluster_rows = row_dend, name = "Types of Selection",show_row_names = FALSE, show_column_names = F,border = TRUE,show_heatmap_legend = FALSE,show_row_dend = FALSE,
                  column_names_gp = gpar(fontsize =  5, fontface = "bold"), cluster_columns = FALSE,col=col_dim2,column_title="chi_square Selection", column_title_gp =gpar(fontsize =  5, fontface = "bold"),
                  bottom_annotation = ha) #col = col_fun1,
    ...
    ##Dendrogram only for if Group==ALL
        #if (Groups[g]=="All") {
          #MethCores$Groups=MethCores$Group
          ...
          dendrogram1$Samples=dendrogram1$Group
          dendrogram1$Group=MethCores$Group
          ...
          dendrogram1=dendrogram1[,c("MethCores","Samples")]
          colnames(dendrogram1)=c("core","Samples")
          ...
          dendrogram3$strings=apply( dendrogram3[ ,2:(dim(dendrogram3)[2]) ] , 1 , paste , collapse = "" )
          ...
          #if (length(d)>1) {
           png(paste("./",destination_folder,"/Phylogenetic_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
              width = 5*250,        # 5 x 300 pixels
              height = 5*250,
        ...
        plot(as.phylo(hcl), main = paste("Phylogenetic_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene,".txt",sep=""),tip.color=colorCodes1$colors, type="cladogram",
        ...
        ##Write phylogram
         ...
         write.hclust(hcl, file =  paste("./",destination_folder,"/Phylogenetic_Cluster_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene,".txt",sep=""), prefix = prefix,h=max(hcl$height)) 
         write.table(x=dm, file=paste("./",destination_folder,"/Table_Phylogenetic_Cluster_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
        ...
        # if (length(hc)>1) {
         # And the plot
         png(paste("./",destination_folder,"/Dendrogram_",Groups[g],"_Epiallels_",select,"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
             width = 5*250,        # 5 x 300 pixels
             height = 5*250,
             res = 500,            # 300 pixels per inch
             pointsize = 4,units = c("px"))        # smaller font size
         par(mar=c(9,9,9,9))
         plot(dhc,horiz = TRUE ,main = paste("Dendrogram_",Groups[g],"_Epiallels_",select,"_",class,"_",Complexity[hh],"_",gene,sep=""),
              cex = 1)
       dev.off()
         #}
        ...
        # #dendrogram1=dendrogram[,c(1:2,4)]
        # ## If MethCores$MethCores
        # dendrogram1=dendrogram1[,c("Group","Groups","MethCores","MethCores_Index","Samples")]
        # colnames(dendrogram1)=c("Group","Groups","MethCore","Freq","Samples")
        # #dendrogram1=dendrogram[,c(1:2,4)]# +Freq
        # 
        # dendrogram1[,"Freq"]= dendrogram1[,"Freq"]*100
        # #dendrogram2 <- cast(dendrogram1, core~Samples, mean,value = 'Freq')
        # #dendrogram2[is.na(dendrogram2)] <- 0
        # dendrogram2=dendrogram1
        # #dendrogram2[,"Random"]=0
        # #dendrogram2=strings_to_BinaryProfiles(df=dendrogram1,CG_pos=CG_pos,class=class)
        # 
        ...
        # 
        # if (length(d)>1) {
        # png(paste("./",destination_folder,"/Dendrogram_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene,".png",sep=""),
        #     width = 5*250,        # 5 x 300 pixels
        #     height = 5*250,
        #     res = 500,            # 300 pixels per inch
        #     pointsize = 4,units = c("px"))        # smaller font size
        ...
        # write.hclust(h, file =  paste("./",destination_folder,"/Dendrogram_Cluster_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene,".txt",sep=""), prefix = prefix,h=max(hcl$height)) 
        # write.table(x=mat, file=paste("./",destination_folder,"/Table_Dendrogram_Cluster_",Groups[g],"_",select,"_",class,"_",Complexity[hh],"_",gene, ".txt", sep=""), row.names=T,col.names=T, quote = F, sep="\t")
        # }
        # }
        #Extract CpGs from MethCores
        MethCoress=MethCores$my_data
        colnames(MethCoress)=c("Rep","MethCores_Index","Groups","MethCores")
        
        #CpGs=unique(unlist(strsplit(MethCores$MethCores, "-")))
        CpGs=as.character(unlist(CG_pos[1,]))
        #Create a empty df
        tab_MethCores=data.frame(CpGs=NA,Groups=NA,MethCores_Index=-1,id=NA,Rep=NA)
        tab_MethCores=tab_MethCores[0,]
        
        #MethCores=MethCores$MethCores
        for (c in 1:nrow(MethCoress)) {
          tab=MethCoress[c,]
          tab_cores=unlist(strsplit(tab$MethCores, "-"))
          
          tab_Meth=data.frame(matrix(NA,ncol=4,nrow=length(tab_cores)))
          colnames(tab_Meth)=c("CpGs","Groups","MethCores_Index","Rep")
          
          #Create a empty df
          # MethCores_Index=percent of cores in total population
          tab_Meth$CpGs=tab_cores
          tab_Meth$MethCores_Index=tab$MethCores_Index
          tab_Meth$Rep=tab$Rep
          ...
          if (length(tab_cores)==length(CpGs)) {
           ...
           # } else {
            ...
            tab_Meth$Groups=tab$Group
             ...
             # tab_Meth$id=paste(tab$Groups,tab$Group,sep="_")
            #} else {
              tab_Meth$id=paste(tab$Group,tab$Rep,sep="_")
            #}
        ...
        cores=CpGs
        Stat$Group=as.character(Stat$Groups)
        
        if (Groups[g]=="All") {
          gStat=Stat
          gStat=gStat[!grepl("R$", gStat$Groups),]
        } else {
        ...
        aggregateGroups=aggregate(gStat[,1:(dim(CG_pos)[2])], list(gStat$labels), mean)
        ...
        aggregateGroups_cores$Rep=sub(".*_","",aggregateGroups_cores$Groups)
        ...
        #aggregateGroups_cores$id=paste( aggregateGroups_cores$Family, aggregateGroups_cores$Groups,sep="_")
        tab_MethCores=merge(tab_MethCores,aggregateGroups_cores,by=c("Groups","CpGs","id","Rep"),all=T)
        ...
        tab_MethCores=tab_MethCores[tab_MethCores$Family!=0,]
        ...
        yearly_counts <- tab_MethCores %>% group_by(CpGs, Groups,CpG_Variation,Family,id,Rep) %>% tally
        yearly_counts <- yearly_counts[yearly_counts$CpG_Variation != 0,]
        #if (max(yearly_counts$n)==1) next
        
        #if (length(unique(yearly_counts$Groups))==length(unique(yearly_counts$Family))) {
        if (overtime==TRUE) { 
          X=factor(yearly_counts$Rep,levels = c("S1","S2","S3","S4","S5","S6","S7","S8","S9","S10"))
          yearly_counts$CpGs=as.character(yearly_counts$CpGs)
          X1="Rep"
        } else {
        ...
        j2=ggplot(yearly_counts,aes(X,CpG_Variation, group=CpGs)) + geom_point(aes(colour=CpGs),position = "stack") + geom_line(aes(colour=CpGs),position = "stack")+
          facet_wrap(~ Family+CpGs, scales="free_x") +
        ...
        rm(list = ls()[!ls() %in% c("colorCodes","destination_folder","overtime","Epi_list1","population_weight","MethCores_tab2","cores_cutOff","MethCoresIndex","MethCores_tab","Assign_color","MethCores1","MethCores",
