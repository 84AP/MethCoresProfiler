MethCores_tab2 <- function(timing,t,tdim5,tmap1,g,filename,Complexity,hh,timing1,t1,Epi_list1,gene_destination_folder,Groups) {

    MethCores_Index=data.frame(average_methylation=-1,MethCores_Index=-1,Clonality_Index=-1,Group=NA,Groups=NA,Tissue=NA,Gene=NA,MethCores=NA,quantile=-1,sd_MethCores_Index=-1,se_MethCores_Index=-1,sd_Clonality_Index=-1,se_Clonality_Index=-1,MoreRepresentativeEpiallele=-1)
  tab_Stat_MethCores=data.frame(Samples=NA,average_methylation=-1,MethCores_Index=-1,Clonality_Index=-1,MethCores=NA,average_MethCores_Index=-1,sd_MethCores_Index=-1,se_MethCores_Index=-1,average_Clonality_Index=-1,sd_Clonality_Index=-1,se_Clonality_Index=-1,MoreRepresentativeEpiallele=-1)
  ...
  if (population_weight==TRUE) {
    ...
    population2a=my_data3
    population3=my_data3[0,]
    
    #if (dim(population3)[1]!=length(population)) {
    #  population3=population3[rep(seq_len(nrow(population3)), each = length(population)), ]
    #} else {
    #  population3=population3
    #}
    
    for (pop in 1:length(population)) {
    population1=read.table(paste(input_folder2,gene,"/",population[pop],sep=""), header=TRUE, sep="\t",stringsAsFactors=FALSE)
    ...
    population2a$weight=sum(population2$FreqCloneForSample)
    population3=rbind(population2a,population3)    
    }
    my_data3$weight=mean(population3$weight)
  ...
  if (max(my_data3$weight)>=vertex_cutOFF) {
    ...
    vertex1=my_data3
    vertex1=vertex1[!vertex1$weight <= vertex_cutOFF,]
  } else {
  ...
  #if (length(tab_vertex1)!=0) {
  #tab_vertex1=stri_remove_empty(tab_vertex1)
  #tab_vertex1=stri_remove(tab_vertex1)
  ...
  #vertex1=my_data3
  write.table(x= my_data3, file=paste(gene_destination_folder,"/Table_Network_Epialleles_",filename,"_",timing[t],"_",class,"_",gene, ".txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
  ...
  ## Check 
  #sum(z, na.rm = TRUE)
  if (("" %in% links$to) == FALSE) { #links[links$to %in% "" ] >=1)
    vertex=vertex
  } else {
    #add empty 
  addEmptyvertex=sum((links$to %in% ""), na.rm = TRUE)
    vertex[nrow(vertex)+addEmptyvertex,] <- ""
  #} else {
  #links[which(! links$From %in% vertex$vertex) ,]
  #links[which(! links$to %in% vertex$vertex) ,]
  #  vertex=vertex
  }
  ...
  sz <- as.numeric(vertex$Freq[match(V(network)$name, vertex$vertex)])
  ...
  co <- layout_with_fr(network, minx=minC, maxx=maxC,
                       miny=minC, maxy=maxC)
  # Plot
  png(paste(gene_destination_folder,"/Cores_graph_",filename,"_",timing[t],"_",class,"_",gene,".png",sep=""),
  ...
  for (dd in 1:length(Samples)) {
    ddim5=tdim5[tdim5$Samples %in% Samples[dd], ]
    ...
    tab_Stat_cores$MethCores_Index[dd]=Stat_cores$Freq
  ...
  tab_Stat_cores$Clonality_Index=as.numeric(tab_Stat_cores$MethCores_Index)/as.numeric(tab_Stat_cores$average_methylation)
  ...
  MethCore_Index=data.frame(average_methylation=-1,MethCores_Index=-1,Groups=NA,Group=NA,Tissue=NA,Gene=NA,MethCores=NA,quantile=-1,MoreRepresentativeEpiallele=-1)
  ...
  MethCore_Index$Clonality_Index=as.numeric(MethCore_Index$MethCores_Index)/as.numeric(MethCore_Index$average_methylation)
  MethCore_Index$Group=timing[t]
  MethCore_Index$Groups=timing1[t1]#unique(Stat1$Tissue[MethCore_Index$Group == Stat1$Group])
  ...
  MethCores_tab1=list(tab_Stat_MethCores=(tab_Stat_MethCores),MethCores_Index=(MethCores_Index))
  rm(my_data2,my_data3,network,vertex,vertex1)
