cores_status_assignment <- function(str1,str2,Complexity,Group,hh,g,tmap1,maxCG_pos,Stat,vertex_cutOFF,dim1,Epi_list1,gene_destination_folder,Groups) {
  ...
  #if there qare not significant epialleles, takes that more frequently
  if (dim(dim4)[1]!=0) {
     dim4=dim4
  ...
  dim4$core=as.character(rownames(dim4))
...
#par(mar = c(4, 4, 4, 6),    xpd = TRUE)                              # Specify par parameters
...
# 
 ...
 MethCores=list(MethCores=MethCores_Index,dim5=dim5)
     ...
     MethCores_Index=MethCores_Index[0,]
     
     dim5=data.frame(core=NA,Samples=NA,Selection=NA,Freq=-1,Group=NA,Tissue=NA)
     dim5=dim5[0,]

     #dimm4=as.data.frame(str1)
     #for (nncol in 1:ncol(dimm4)) {
     #   dimm4=dimm4[order(dimm4[,nncol],decreasing = TRUE), ]
    #    dimm4[,nncol]="white"
     #   dimm4[1,nncol]="red"
     #} 
     #sel1 <- apply(dimm4[,1:(dim(dimm4)[2])],1,function(row) "red" %in% row)
    # dim4=dimm4[sel1,]
     #dimm3=dim3
     #for (nncol in 1:ncol(dimm3)) {
     #     dimm3[,nncol]="red"
     #}
     #sel1 <- apply(dimm3[,1:(dim(dimm3)[2])],1,function(row) "red" %in% row)
     #dim4=dimm3[sel1,]
    # dim4[,"Random"]="white"
     MethCores=list(MethCores=MethCores_Index,dim5=dim5)
