...
cores_status_assignment <- function(str1,str2,Complexity,Group,hh,g,tmap1,maxCG_pos,Stat,vertex_cutOFF,dim1,Epi_list1) {
  ...
  # Tieni solo le righe che sono positive in tuti i campioni del gruppo
 ...
 dim5$Group=sub("_.*","",dim5$Samples)
 dim5$Tissue="synthetic"
 ...
 #nummero di duplicati per colonna
 times=max(unique(sub("S","",tmap1$Rep)))
...
png(paste("./",destination_folder,"/Groups_Significative_",Groups[g],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
...
# 
 ...
 timing=unique(as.vector(tmap1$Description))[-length(unique(tmap1$Description))]
   ...
   if (Groups[g]=="All") {
     ...
     MethCores_tab1=MethCores_tab2(tdim5=dim6,timing,t,tmap1,g,filename,Complexity,hh,timing1,t1)
    ...
    # tab_Stat_MethCores=tab_Stat_MethCores2
     #MethCores_Index=MethCores_Index2
       ...
       MethCores_Index2=data.frame(average_methylation=-1,MethCores_Index=-1,Clonality_Index=-1,Group=NA,Groups=NA,Tissue=NA,Gene=NA,MethCores=NA,quantile=-1,sd_MethCores_Index=-1,se_MethCores_Index=-1,sd_Clonality_Index=-1,se_Clonality_Index=-1)
       tab_Stat_MethCores2=data.frame(Samples=NA,average_methylation=-1,MethCores_Index=-1,Clonality_Index=-1,MethCores=NA,average_MethCores_Index=-1,sd_MethCores_Index=-1,se_MethCores_Index=-1,average_Clonality_Index=-1,sd_Clonality_Index=-1,se_Clonality_Index=-1)
       tab_my_data1= data.frame(Rep=NA,Freq=-1,Group=NA,core=NA)
       
     for (t in 1:length(timing)) { 
     filename=paste(Groups[g],sep="_")
     MethCores_tab1=MethCores_tab2(tdim5=dim5,timing,t,tmap1,g,filename,Complexity,hh,timing1,t1,Epi_list1)
  
     tab_Stat_MethCores1=MethCores_tab1$tab_Stat_MethCores
     MethCores_Index1=MethCores_tab1$MethCores_Index
     tab_my_data=MethCores_tab1$my_data
     
     tab_Stat_MethCores2=rbind(tab_Stat_MethCores2,tab_Stat_MethCores1)
     MethCores_Index2=rbind(MethCores_Index2,MethCores_Index1)
     #tab_my_data=MethCores_tab1$my_data
     tab_my_data1=rbind(tab_my_data1,tab_my_data)
     
     }
   }
 tab_Stat_MethCores= tab_Stat_MethCores2
 MethCores_Index=MethCores_Index2
 tab_my_data1=tab_my_data1[!is.na(tab_my_data1$Group),]

 tab_Stat_MethCores=tab_Stat_MethCores[!is.na(tab_Stat_MethCores$Samples),]
 write.table(x= tab_Stat_MethCores, file=paste("./",destination_folder,"/Table_Stat_MethCores_index_",Groups[g],"_",class,"_",gene,".txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
 ...
 write.table(x= MethCores_Index, file=paste("./",destination_folder,"/Table_MethCores_index_",Groups[g],"_",class,"_",gene,".txt", sep=""), row.names=F,col.names=T, quote = F, sep="\t")
 
 if (Groups[g]=="All") {
 MethCores_Index$MethCores_Index=round(MethCores_Index$MethCores_Index,digits = 2)
 ...
 # Clonality Index
 png(paste("./",destination_folder,"/Clonality_Index_",Groups[g],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
 ...
 png(paste("./",destination_folder,"/MethCores_Index_",Groups[g],"_",class,"_",gene,".png",sep=""), width = 5*300,height = 5*300,res = 300,pointsize = 8) 
 ...
 MethCores=list(MethCores=MethCores_Index,dim5=dim5,my_data=tab_my_data1)
  return(MethCores)
}
