make_graph<- function(tmer,tmer2,sam,summ_stat,gene_destination_folder) {
  #Methylated Grafo
  #Create a df
  tab_mer2=tmer2
  ...
  #}
    if (saveAs=="pdf") {
      pdf(paste(gene_destination_folder,"/",sam,"_",class,"_Graph_", gene,".pdf", sep=""),paper = "a4")
    } else { 
  png(paste(gene_destination_folder,"/",sam,"_",class,"_Graph_", gene,".png",sep=""),width = 5*150,height = 5*150,res = 200,pointsize = 4)        # smaller font size
    }
  ...
  write.table(x = Positive_crl, file = paste(gene_destination_folder,"/",sam,"_Positive_Selection_",class,"_clust_", gene, ".txt",sep=""), quote=F,sep="\t",row.names=F, col.names = T)
  write.table(x = Negative_crl, file = paste(gene_destination_folder,"/",sam,"_Negative_Selection_",class,"_clust_", gene, ".txt",sep=""), quote=F,sep="\t",row.names=F, col.names = T)
  write.table(x = crl, file = paste(gene_destination_folder,"/",sam,"_All_Links_",class,"_clust_", gene, ".txt",sep=""), quote=F,sep="\t",row.names=F, col.names = T)
}
