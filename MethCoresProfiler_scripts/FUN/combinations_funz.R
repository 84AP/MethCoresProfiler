...
combinations<- function(cl,tab_Stat) {
 ...
 #Assign Selection Type Respect to Exp_pvalue
  ...
  #write file
  write.table(x=summary1, file=paste("./",destination_folder,"/","ComposCore_",dim_n_cores1,"_",class,"_",sam,"_",Clust_Type,"_" ,gene,".txt", sep=""), row.names=F, quote = F, sep="\t")
  write.table(x=summ_clust1, file=paste("./",destination_folder,"/","Tab_Epialleli_",dim_n_cores1,"_",class,"_", sam, "_",Clust_Type,"_",gene,".txt", sep=""), row.names=F, quote = F, sep="\t")
