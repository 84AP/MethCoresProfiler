...
MethCores_tab <- function(timing,t,tdim5,tmap1,g,filename,Complexity,hh,timing1,t1) {
  ...
  Stat1=Stat[Stat$Group %in% samples,]
  # if there are is not red take a more frequently
  if ("red" %in% vertex$colors) {
  ...
  for (dd in 1:length(Samples)) {
    ...
    tab_Stat_cores$MethCores_Index[dd]=sum(ddim6$Freq)
